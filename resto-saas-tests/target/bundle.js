var gatling = (() => {
  ...
  var require_assertions = __commonJS({
    "node_modules/@gatling.io/core/target/assertions.js"(exports) {
      ...
      var details2 = (...parts) => wrapAssertionWithPath(jvm_types_1.CoreDsl.details(...parts));
  ...
  __export(stdin_exports, {
    "resto-saas": () => resto_saas_gatling_default
  });
  init_globals();

  // src/simulations/resto-saas.gatling.js
  init_globals();
  var import_core = __toESM(require_target(), 1);
  var import_http = __toESM(require_target2(), 1);
  var BASE_URL = "https://www.resto-saas.com";
  var RESTAURANT_SLUG = "petit-plat";
  var TABLE_NUMBER = "1";
  var resto_saas_gatling_default = (0, import_core.simulation)((setUp) => {
    const httpProtocol = import_http.http.baseUrl(BASE_URL).acceptHeader(
      "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"
    ).acceptLanguageHeader("fr-FR,fr;q=0.9,en;q=0.8").acceptEncodingHeader("gzip, deflate, br").userAgentHeader("Gatling-Spike-Test-v2/1.0").disableFollowRedirect();
    const scnSpikeLanding = (0, import_core.scenario)("SPIKE - Landing page x100").exec(
      (0, import_http.http)("01 Landing page").get("/").check((0, import_http.status)().in(200, 304))
    );
    const scnSpikeMenu = (0, import_core.scenario)("SPIKE - Menu restaurant x100").exec(
      (0, import_http.http)("02 Menu restaurant").get(`/r/${RESTAURANT_SLUG}?table=${TABLE_NUMBER}`).check((0, import_http.status)().in(200, 304))
    );
    const scnSpikePanier = (0, import_core.scenario)("SPIKE - Panier x100").exec(
      (0, import_http.http)("03 Panier").get(`/r/${RESTAURANT_SLUG}/cart?table=${TABLE_NUMBER}`).check((0, import_http.status)().in(200, 304, 404))
    );
    const scnSpikeApi = (0, import_core.scenario)("SPIKE - API keepalive x100").exec(
      (0, import_http.http)("04 API keepalive").get("/api/keepalive").header("Accept", "application/json").check((0, import_http.status)().in(200, 404))
    );
    setUp(
      scnSpikeLanding.injectOpen((0, import_core.atOnceUsers)(100)),
      scnSpikeMenu.injectOpen((0, import_core.atOnceUsers)(100)),
      scnSpikePanier.injectOpen((0, import_core.atOnceUsers)(100)),
      scnSpikeApi.injectOpen((0, import_core.atOnceUsers)(100))
    ).protocols(httpProtocol).assertions(
      (0, import_core.global)().responseTime().percentile(95).lt(1e4),
      (0, import_core.global)().successfulRequests().percent().gt(80),
      (0, import_core.details)("01 Landing page").responseTime().percentile(95).lt(8e3),
      (0, import_core.details)("02 Menu restaurant").responseTime().percentile(95).lt(1e4)
    );
  });
