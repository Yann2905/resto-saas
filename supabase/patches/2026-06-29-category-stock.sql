-- Category-level stock management
-- stock is nullable: NULL = no stock management for this category
ALTER TABLE categories ADD COLUMN IF NOT EXISTS stock numeric DEFAULT NULL;

-- Per-product consumption: how much category stock this product uses per unit ordered
ALTER TABLE products ADD COLUMN IF NOT EXISTS stock_consumption numeric NOT NULL DEFAULT 1;

-- Update create_order RPC to handle category-level stock
...
BEGIN
  ...
  -- First pass: validate all items and accumulate category deductions
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Check if category has stock management
    v_category := NULL;
    ...
    IF v_category IS NOT NULL AND v_category.stock IS NOT NULL THEN
      -- Category stock management: accumulate total deduction per category
    ...
    ELSE
      -- Product-level stock (legacy behavior)
  ...
  -- Apply product-level stock deductions (for products without category stock)
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    v_pid := (v_item->>'productId')::uuid;
    v_qty := (v_item->>'quantity')::int;
    ...
    -- Only deduct product stock if category does NOT have stock management
